...
const Solitaire = () => {
			...
			for (let j = i; j < 8; j++) {
				setTimeout(() => {
					setTimeout(() => {
						...
						const card = stock.moveCardToDeck(
							tableaus[`tableau-${j}`]
						);
					}, j * 100 - i * 25);
				}, i * 600);
	...
	function addMultipleCardsToTableaus(source, destination, card) {
		const cardIndex = source.deck.cards.findIndex(
			(index) => index === card
		);
		for (let index = cardIndex; index < source.deck.cards.length; index++) {
			setTimeout(() => {
				...
				cardMoved.card.removeEventListener(
					"click",
					cardMoved.boundListener
				);
