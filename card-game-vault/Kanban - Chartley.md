...
- [ ] Separate card logic into its on module, so it can be imported by Dealer.
...
- [ ] Refactor code to be composition based. Separate `card` from `playing`. This will make creating future card sets far simpler. A `card `should just have features for size, front, back, flip, etc. Where as `playing` should have numbers, suits, etc.
